...
define(['exports', 'jquery', 'ojs/ojcore-base', 'ojs/ojdomutils'], function (exports, $, oj, DomUtils) { 'use strict';
   ...
   * focusout, but component don't want to exit actionable/edit mode in those cases.
   * This method should be used inside the component's focusout handler.
   * @param elem the component element
   * @return true if there is an opened popup triggered from within the component, false otherwise.
   */
  DataCollectionUtils.isContainOpenPopup = function (elem) {
    ...
    for (var i = 0; i < popups.length; i++) {
      ...
      // Check if the listview contains the launcher
      if (launcher != null && $(elem).has(launcher.get(0)).length > 0) {
  ...
  const isContainOpenPopup = DataCollectionUtils.isContainOpenPopup;
  ...
  exports.isContainOpenPopup = isContainOpenPopup;
